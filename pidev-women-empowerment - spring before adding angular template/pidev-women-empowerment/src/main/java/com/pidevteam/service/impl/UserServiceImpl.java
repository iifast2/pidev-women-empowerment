...
public class UserServiceImpl implements UserDetailsService, UserService {
	...
	public User save(UserDto user) {
		...
		{		newUser.setPassword(bcryptEncoder.encode(user.getPassword()));
			if(user.getEmail() != null){
				emailSenderService.sendMail(user.getEmail(), "this is an email!  \n " +"your username : "+ user.getUsername() + "\n your user password : " + user.getPassword(), " \nWomen empowerment user infos ");
				//https://i.imgur.com/7s5Jv7k.png
			}
			if(user.getPhone() != null){
				smsController.smsSubmit(new SmsPojo(user.getPhone(),"Welcome to Women Empowerment app - \n your user password : " + user.getPassword() +"  \n and your username "+user.getUsername()));
			}
	...
	public boolean changePassword(ChangePasswordVM vm , String username){
		...
		if (bcryptEncoder.matches(vm.getOldPassword(),user.getPassword())){
			...
			emailSenderService.sendMail(user.getEmail(), "this is an email!  \n \n your user NEW password : " + vm.getNewPassword() + "\n YOUR OLD PASSWORD : \n" + vm.getOldPassword(), "Women empowerment user infos ");
